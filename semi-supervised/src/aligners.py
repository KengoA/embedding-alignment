...
def bdr_0_0_1(x, y, logger, log_per_epoch=True, y_idx_map=None, n_dim=2, max_epoch=50, sup_size=0.05, n_restart=1):
    ...
    loss_cycle_scale = tf.constant(2000., dtype=K.floatx()) # TODO
    # loss_cycle_scale = tf.constant(1000, dtype=K.floatx()) # TODO
    gmm_scale = tf.constant(.01, dtype=K.floatx())

    loss_sup_scale = tf.constant(5000., dtype=K.floatx()) # TODO
    # sup_size = 0.05
    ...
    # @tf.function
    def train_supervised_step(x_all, y_all, loss_sup_scale):
        with tf.GradientTape() as tape_sup_f:
            f_x_sup = model_f(x_all[x_sup_idx])
            sup_loss_f = sup_loss_func(y_all[y_sup_idx], f_x_sup, loss_sup_scale)
        gradients_sup_f = tape_sup_f.gradient(sup_loss_f, model_f.trainable_variables)
        del tape_sup_f

        with tf.GradientTape() as tape_sup_g:
            f_y_sup = model_g(y_all[y_sup_idx])
            sup_loss_g = sup_loss_func(x_all[x_sup_idx], f_y_sup, loss_sup_scale)
        gradients_sup_g = tape_sup_g.gradient(sup_loss_g, model_g.trainable_variables)
        del tape_sup_g

        optimizer.apply_gradients(
            zip(gradients_sup_f, model_f.trainable_variables)
        )
        optimizer.apply_gradients(
            zip(gradients_sup_g, model_g.trainable_variables)
        )

        return sup_loss_f, sup_loss_g
    ...
    for i_restart in range(n_restart):
        ...
        # Start algorithm.
        template_loss = '    Epoch {0} Loss | total: {1:.5g} | cycle: {2:.3g} | f_dist: {3:.3g} | g_dist: {4:.3g} | f_sup: {5:.3g} | g_sup: {6:.3g}'
        ...
        for i_epoch in range(max_epoch):
            ...
            if sup_size > 0:
                sup_loss_f, sup_loss_g = train_supervised_step(x, y, loss_sup_scale)
            else:
                sup_loss_f = 0
                sup_loss_g = 0
            ...
            if np.mod(i_epoch, 1) == 0:
                ...
                print(
                    template_loss.format(
                        ...
                        dist_loss_f, dist_loss_g, sup_loss_f, sup_loss_g
                    )
                )
                if y_idx_map is not None:
                    print(
        ...
        if y_idx_map is not None:
            ...
            logger.info(
                ...
                template_loss.format(i_epoch+1, loss_total, cycle_loss, dist_loss_f, dist_loss_g, sup_loss_f, sup_loss_g)
                )
...
def sup_loss_func(x_sup, f_y_sup, loss_sup_scale):
    ...
    loss = loss_sup_scale * tf.reduce_mean(d_x)
    return loss
