...
def highway_layer(input_layer, input_dim, hidden_dim, output_dim, Wg=None, bg=None, Ws=None, bs=None, name='highway'):
    with tf.compat.v1.name_scope(name + "-gate-layer"):
        if Wg is None:
            Wg = tf.Variable(tf.random.truncated_normal([input_dim, input_dim], stddev=0.05), name="W")
        if bg is None:
            bg = tf.Variable(tf.random.truncated_normal([input_dim], stddev=0.05), name="b")
    ...
    with tf.compat.v1.name_scope(name):
        for i, d_i in enumerate(dims[:-1]):
            ...
            if add_multiple_noise is not None:
                ...
                gaussian_noise = tf.random.normal(shape = tf.shape(input=layers[i]), mean = 1.0, stddev = add_multiple_noise, dtype = tf.float32)
            ...
            if Ws[i] is None:
                ...
                Ws[i] = tf.compat.v1.get_variable(name="W"+str(i)+name, shape=[d_i, d_o], dtype=tf.float32, initializer=initializer)
            ...
            layers[i+1] = tf.compat.v1.nn.xw_plus_b(layers[i], Ws[i], bs[i], name="fc_out"+str(i+1))
...
def add_multiple_gaussian_noise(inp_tensor, stddev):
    gaussian_noise = tf.random.normal(shape = tf.shape(input=inp_tensor), mean = 1.0, stddev = stddev, dtype = tf.float32)
