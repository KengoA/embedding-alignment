...
def gradient_norm_from_one(d_hat, x_hat, scale):
    ddx = tf.gradients(ys=d_hat, xs=x_hat)[0]
    ddx = tf.sqrt(tf.reduce_sum(input_tensor=tf.square(ddx), axis=1))
    ddx = tf.reduce_mean(input_tensor=tf.square(ddx - 1.0) * scale)
    return ddx
...
def set_W_gan_layers(true_tensor, fake_tensor, input_dim, disc_hidden_dim, scale, name):
    ...
    gen_label = tf.ones([tf.shape(input=fake_tensor)[0]], dtype=tf.float32)
    disc_label = tf.concat([tf.ones([tf.shape(input=true_tensor)[0]], dtype=tf.float32),-tf.ones([tf.shape(input=fake_tensor)[0]], dtype=tf.float32)], 0)
    l_disc = tf.reduce_mean(input_tensor=tf.multiply(tf.squeeze(disc_logits), disc_label))
    l_gen = tf.reduce_mean(input_tensor=tf.multiply(tf.squeeze(gen_logits), gen_label))

    # Improved Training of Wasserstein GANs
    epsilon = tf.random.uniform(tf.shape(input=true_tensor), 0.0, 1.0)
